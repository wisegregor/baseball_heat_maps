...
After realizing the data I scraped included pitchers with PA’s, I wanted to figure out a way to omit those players from the data set. I found that the [“expected_statistics”](https://baseballsavant.mlb.com/leaderboard/expected_statistics?type=pitcher) leaderboard which is extractable from Baseball Savant, contained both player positions and plate appearance data. I scraped all non-pitcher data and combined the data together. I tried setting the position parameter to position != 1, but that did not work out.
...
$batterstrikeoutpct = zoneswingpct + outofzonemisspct + inzonecontactpct + nonzonepct$
...
Here is where I calculated first pitch strike percentage, but it was ultimately not used in our final model.
...
$batterwalkmodel1 = batterwalkpct + outofzoneswingpct + batterzonepct + firstpitchstrikepct$
...
$pitcherkpct = pitcherexpectedstrikepercentage + inzonelookingstrikepct + inzonewhiffpct + inzonefoulpct$

Firstly, we need to scrape in only pitcher data from baseball savant using the baseballr package.
...
$pitcherBBpct = pitcherexpectedstrikepercentage + pitcherballinplayperexpectedstrikepercentage$
...
Finally, using [this variation](https://ieeexplore.ieee.org/document/7069266) of the Log 5 equation, we'll be able to adjust strikeout and walk rates based on the expected strikeout and walk rates of each hitter-pitcher match-up.

The equation I used is as follows: $E* = (BP)/L / (BP)/L + (1-B)(1-P)/(1-L)$
* We let B be a batter's strikeout percentage
* We let P be a pitcher's strikeout percentage
* We let L be the league average strikeout percentage. The log5 method then predicts the probability of E* being a strikeout for a matchup between a batter and a pitcher. This same model is used for walk probability.

The league strikeout and walk rates, calculated from [this](https://www.fangraphs.com/leaders.aspx?pos=all&stats=bat&lg=all&qual=0&type=c,16,6&season=2020&month=0&season1=2017&ind=0&team=0,ss&rost=0&age=0&filter=&players=0&startdate=&enddate=) Fangraph link, are as follows:

* 2017 Batter Strikeout Rate: 21.6%
* 2017 Pitcher Walk Rate: 8.0%

* 2018 Batter Strikeout Rate: 22.3%
* 2018 Pitcher Walk Rate: 8.5%

* 2019 Batter Strikeout Rate: 23.0%
* 2019 Pitcher Walk Rate: 8.5%

* 2020 Batter Strikeout Rate: 23.4%
* 2020 Pitcher Walk Rate: 9.2%
...
final_join_with_real_rates <- left_join(final_join, test_data1, by = c("player_name", "batter", "game_year"))%>%
  ...
  select(game_year, player_name, batter_strikeout_pct, Adjusted_SO_pct, batter_walk_pct, Adjusted_BB_pct)
...
final_join_with_real_rates%>%
  rename(Year = game_year, Batter = player_name, K_pct = batter_strikeout_pct, Adj_K_pct = Adjusted_SO_pct, BB_pct = batter_walk_pct, Adj_BB_pct = Adjusted_BB_pct)->final_join_with_real_rates

final_join_with_real_rates

# Full results can be found on my Github "adjusted_strikeout_and_walk_rates.csv"
```
